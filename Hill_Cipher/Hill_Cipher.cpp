...
{
    vc.resize(3, vector<int>(3));   // encryption matrix
    vc2.resize(3, vector<int>(3));  // decryption key(inverse)
    c1.resize(3, vector<int>(1));   // plaintext
    c2.resize(3, vector<int>(1));   // ciphertext
    c3.resize(3, vector<int>(1));   // decrypted text
    srand((unsigned)time(nullptr)); // for random key generation
}

// compute adjoint of 3x3 int matrix
